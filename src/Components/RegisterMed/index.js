...
const genderReducer = (state, action) => {
  if (action.type === 'USER_INPUT') {
    return { value: action.val, isValid: action.val === "masculino" || action.val === "feminino"};
  }
  if (action.type === 'INPUT_BLUR') {
    return { value: state.value, isValid:  action.val === "masculino" || action.val === "feminino"};
  }
  return { value: '', isValid: false };
};
...
export default function RegisterMed() {
  ...
  const validateGenderHandler = () => {
    dispatchGender({ type: 'INPUT_BLUR' });
  };
  ...
  const submitHandler = (event) => {
    ...
    const data = {
      ...
      telefone_secundario: phoneState.value,
      celular: cellphoneState.value,
